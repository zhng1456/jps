...
class PreCalculatedShortestPathCalculator extends ShortestPathCalculator {
    ...
    PreCalculatedShortestPathCalculator(PreCalculationShortestPathPreprocessing preprocessing,
            ShortestPathPruning pruning) {
        ...
        if (preprocessedPoint != null) {
            ...
            if (steps > 0) {
                ...
                if (forcedPoint.equals(goal))
                    return new Tuple2<>(forcedPoint,
                            cost + steps * Math.sqrt(Math.abs(direction.getX()) + Math.abs(direction.getY())));
                for (Vector subordinatedDirection : movingRule.getSubordinatedDirections(direction)) {
                    Tuple2<Vector, Double> NextInDir = exploreStrategy(map, forcedPoint, subordinatedDirection, 0.0,
                            goal, movingRule);
