...
/**
 * ShortestPathCalculatorStrategy provides the opportunity to memorise a
 * specific ShortestPathCalculator implementation to use it for finding a
 * shortest path on a grid map whenever needed.
 *
 * @author Patrick Loka
 * @version 1.0
...
public class ShortestPathCalculatorStrategy {
    ...
    /**
     * Runs the preprocessing on the memorised ShortestPathCalculator
     * implementation.
     *
     * @param map
     *            grid map, which should be preprocessed.
     * @param movingRule
     *            allowed moving rules on the map.
    ...
    /**
     * Searches the shortests path on the given map by the memorised
     * ShortestPahCalculator implementation.
     *
     * @param map
     *            grid map, on whith the search is executed.
     * @param start
     *            start point on the map.
     * @param goal
     *            goal point on the map.
     * @param heuristic
     *            heuristic to estimate the distance between two points on the
     *            map. This has influence on the processing order of exploring
     *            the map.
     * @param movingRule
     ...
     *             or goal are unpassable.
     * @see ShortestPathCalculator
     * @since 1.0
     */
    public Tuple2<ShortestPathResult, Long> run(MapFacade map, Vector start, Vector goal, Heuristic heuristic,
    ...
    public void setJPSPlusShortestPath() {
        this.shortestPathCalculator = new PreCalculatedShortestPathCalculator(
                new JPSPreCalculationShortestPathPreprocessing(), new NoShortestPathPruning());
    }
    ...
    public void setJPSPlusBBShortestPath() {
        this.shortestPathCalculator = new PreCalculatedShortestPathCalculator(
                new JPSPreCalculationShortestPathPreprocessing(), new JPSBoundingBoxesShortestPathPruning());
    }
