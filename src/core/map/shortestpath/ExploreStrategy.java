...
interface ExploreStrategy {
    ...
    /**
     * Defines one step in a grid map exploring strategy returning the next
     * relevant reached point by running in a given direction.<br>
     * <br>
     * A valid implementation returns the next point to process by running from
     * a given point in a given direction, making sure, that intermediate points
     * are unimportant for reaching a given goal considering the implemented
     * strategy.<br>
     *
     * @param map
     ...
     * <br>
     * A valid implementation returns all outgoing directions where the current
     * point is possibly on the shortest path to the goal point.<br>
     *
     * @param map
     *            grid map on which the shortest path search is running.
     * @param currentPoint
     *            currently processed point.
     * @param predecessor
     *            predecessor of the current point.
     * @param movingRule
     *            movement rules for the given grid map.
     * @return Set of directions to explore next starting at the currentPoint.
     * @since 1.0
     */
    Collection<Vector> getDirectionsStrategy(MapFacade map, Vector currentPoint, Vector predecessor,
