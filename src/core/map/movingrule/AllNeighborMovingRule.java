 ...
 * <br>
 * AllNeighborMovingRule implements
 * getForcedDirections(map,currentPoint,direction) of MovingRule by defining
 * every point behind an obstacle, placed aside to the currentPoint, as a forced
 * point.<br>
 * <br>
...
class AllNeighborMovingRule extends MovingRule {
    ...
    /**
     * {@inheritDoc} <br>
     * Implementation: Define every point behind an obstacle, placed aside to
     * the currentPoint, as a forced point.
     *
     * @param map
    ...
    public Collection<Vector> getForcedDirections(MapFacade map, Vector currentPoint, Vector direction) {
        ...
        if (Math.abs(dirX) + Math.abs(dirY) == 2) {
            forcedDirections.addAll(getSubordinatedDirections(direction).stream()
                    .filter(subDir -> !map.isPassable(currentPoint.sub(subDir))
                            && map.isPassable(currentPoint.add(direction).sub(subDir).sub(subDir)))
                    .map(subDir -> direction.sub(subDir).sub(subDir)).collect(Collectors.toList()));
        } else {
            if (dirX == 0) {
