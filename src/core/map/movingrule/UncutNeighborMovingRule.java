...
/**
 * UncutNeighborMovingRule implements all MovingRule operations and overrides
 * one operation.<br>
 * <br>
 * UncutNeighborMovingRule implements
 * getForcedDirections(map,currentPoint,direction) of MovingRule by defining the
 * two points behind an obstacle, placed diagonal behind to the currentPoint, as
 * a forced points.<br>
 * <br>
 * UncutNeighborMovingRule overrides isCornerCut(map,currentPoint,direction) by
 * forbidding Corner Cutting as an accepted movement on the map.<br>
 * <br>
 * All orthogonal movements are allowed as long as the reached point is
 * passable. All diagonal movements are allowed as long as the reached point is
 * passable and no obstacle is cut.
 *
 * @author Patrick Loka
 * @version 1.0
 * @see MovingRule
...
class UncutNeighborMovingRule extends MovingRule {
    ...
    /**
     * {@inheritDoc} <br>
     * Implementation: Define every two points behind an obstacle, placed
     * diagonal behind to the currentPoint, as a forced points.
     *
     * @param map
    ...
    public Collection<Vector> getForcedDirections(MapFacade map, Vector currentPoint, Vector direction) {
        ...
        if (dirY == 0) {
            if (!map.isPassable(new Vector(curX - dirX, curY - 1)) && map.isPassable(new Vector(curX, curY - 1))) {
        ...
        if (dirX == 0) {
            if (!map.isPassable(new Vector(curX - 1, curY - dirY)) && map.isPassable(new Vector(curX - 1, curY))) {
    ...
    /**
     * {@inheritDoc} <br>
     * Implementation(Corner Cutting is forbidden): A Corner is cut, iff there
     * is an obstacle in any subordinatedDirection.
     *
     * @param map
     *            map to apply moving rules.
     * @param currentPoint
     ...
     *            The direction vector in {-1,0,1}^2 in which the currentPoint
     *            is leaving.
     * @return True, if a corner is cut by moving in input direction by leaving
     *         currentPoint. Otherwise return false.
     * @since 1.0
     */
    @Override
    public boolean isCornerCut(MapFacade map, Vector currentPoint, Vector direction) {
        for (Vector subordinatedDirection : getSubordinatedDirections(direction)) {
            if (!map.isPassable(currentPoint.add(subordinatedDirection)))
                return true;
        }
        return false;
    }
