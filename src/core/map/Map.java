 ...
 * <br>
 * Map holds a 2 dimensional boolean array with the information whether a point
 * [x][y] is passable or not. Map holds xDim,yDim as the dimension of this
 * array.<br>
 * <br>
 * Map implements getXDim,getYDim by returning the xDim,yDim attribute.<br>
 * <br>
 * Map implements isPassable(coordinate) by looking up whether the array field
 * with the same x,y values as the coordinate is true or false.
 *
 * @author Patrick Loka
 * @version 1.0
 * @see MapFacade
...
public class Map implements MapFacade {
    ...
    /**
     * Initialisation of a Map instance of a given dimension and a default
     * value.
     *
     * @param dimension
     *            Dimension of the new Map
     * @param areFieldsPassable
     *            Initial value for all points on the map.
     * @throws MapInitialisationException
     *             Thrown if the input dimension has negative values.
     * @see MapFactory
     * @since 1.0
     */
    Map(Vector dimension, boolean areFieldsPassable) throws MapInitialisationException {
        ...
        if (this.xDim < 1 || this.yDim < 1)
            throw new MapInitialisationException(dimension);
        this.map = new boolean[this.xDim][this.yDim];
        if (areFieldsPassable) {
            for (boolean[] col : this.map)
    ...
    /**
     * {@inheritDoc} <br>
     * Implementation: Looks up the value of coordinate in map array and returns
     * value. Returns false if coordinate is no valid input for map array.
     *
     * @param coordinate
     *            point to check whether it is allowed to pass or not
    ...
    /**
     * Switches the passable state in map array of a given point. After running
     * this method, a given passable field becomes unpassable and an given
     * unpassable field becomes passable. If coordinate is no valid input for
     * map array, an InvalidCoordinateException is thrown.
     *
     * @param coordinate
     *            Point where passable state should be switched.
     * @throws InvalidCoordinateException
     *             Thrown if coordinate is no valid input for map array.
     * @see MapController
     * @see MapFacade
     * @since 1.0
     */
    ...
    private void isValideCoordinate(Vector coordinate) throws InvalidCoordinateException {
        if (coordinate.getX() < 0 || coordinate.getY() < 0 || coordinate.getX() >= this.xDim
                || coordinate.getY() >= this.yDim)
